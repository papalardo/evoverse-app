...
class PlayerTicketsListTile extends StatelessWidget {
  ...
  List<DropdownMenuItem<int>> _buildSelect() {
    return kRaffleSelectRange.map((val) {
  ...
  _showModal({required BuildContext context}) {
    showDialog(
      context: context,
      builder: (_context) {
        return AppDialog(
          ...
          child: [
            store.when(
                ...
                "${state.ticketPrice} ${state.ticketCurrency}/ticket",
              ),
            ),
            store.when(
              done: (state) => Obx(() {
                return Text(
                  "${controller.buyAmount() * state.ticketPrice} ${state.ticketCurrency}/total",
                );
              }),
            ),
            const Divider(),
